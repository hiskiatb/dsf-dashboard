...
function calculateGSE(gse) {
  ...
  return {
    actualOno, targetOno, actualGa, targetGa, onoPercent, gaPercent,
...
export default function GSECard({ gse }) {
  ...
  const onoTone =
    c.onoPercent >= 0.8 ? "success" : c.onoPercent >= 0.5 ? "warning" : "danger";
  const gaTone =
    c.gaPercent >= 0.8 ? "success" : c.gaPercent >= 0.5 ? "warning" : "danger";
    ...
    return (
      ...
      >
        <div className="maintenance-body">
          ...
          <div className="maintenance-title">Maintenance Mode</div>
  ...
  return (
    ...
    >
      <div className="card-header">
        <div>
          <div className="card-title">Performance GSE</div>
          <div className="card-desc">Monitoring pencapaian ONO &amp; GA</div>
        </div>
      ...
      <div className="grid-2 mt-4">
        <div className="stat">
          ...
          <div className="stat-value">{gse.idGse}</div>
        </div>
        <div className="stat">
          <div className="stat-label">Nama GSE</div>
          <div className="stat-value">{gse.namaGse}</div>
        </div>
      </div>
      ...
      <details className="details">
        ...
        <div className="grid-2 mt-4">
          <div className="stat">
            ...
            <div className="stat-value">{gse.region}</div>
          </div>
          <div className="stat">
            <div className="stat-label">Branch</div>
            <div className="stat-value">{gse.branch}</div>
          </div>
          <div className="stat">
            <div className="stat-label">Micro Cluster</div>
            <div className="stat-value">{gse.microCluster}</div>
          </div>
        </div>
      </details>
      ...
      <div className="dash-grid mt-5">
        ...
        <div className="dash-right">
          <div className="mini-card">
            ...
            <div className="mini-value">{c.totalSellIn}</div>
          </div>
          <div className="mini-card">
            <div className="mini-label">Total GA</div>
            <div className="mini-value">{c.totalGA}</div>
          </div>
          <div className="mini-card">
            <div className="mini-label">Incentive Sell In</div>
            <div className="mini-value">{formatIDR(c.incSellIn)}</div>
          </div>
          <div className="mini-card strong">
            <div className="mini-label">Total Incentive</div>
            <div className="mini-value">{formatIDR(c.totalIncentive)}</div>
          </div>
        </div>
      </div>
    </motion.div>
